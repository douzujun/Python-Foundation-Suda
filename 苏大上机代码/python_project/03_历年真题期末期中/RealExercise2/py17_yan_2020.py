...
def calcTimeDiff(start, end):
    return time2int(end) - time2int(start)

# 构造车辆累计停留时间的字典
def count_t(vehicle_lst, vehicle_set):
    vehicles = {}
    for elem in vehicle_lst:         # 整条记录:FY-806-579|2016-06-11#10:50:28|2016-06-11#21:48:50
        veh = elem.split('|')[0]     # 车的编号
        time = re.findall('\d+:\d+:\d+', elem)
        diff = calcTimeDiff(time[0], time[1])
        vehicles[veh] = vehicles.get(veh, 0) + diff   # 计算 累计停留时间
...
def write_to_file(vehicle_lst, fre_dict, inter_dict, url):
    with open(url, 'r+', encoding='utf8') as f:
        ...
        # 在校园中累计停留时间最长的5辆车
        f.write("在校园中累计停留时间最长的5辆车（单位：秒）\n")
        for elem in inter_dict:
            f.write("{0}, {1}\n".format(str(elem[0]), str(elem[1])))


def main():
    vehicle_lst = get_record("./file/file_2019yan_input.txt")          # 读文件，获取全部ETC记录，构成列表
